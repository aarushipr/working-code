...
{
	T c = T(state.input->stability_factor);
	helper.AddValue((finger.metacarpal.swing.x - finger_last.metacarpal.swing.x) * kStabilityFingerMCPSwing * c);

	helper.AddValue((finger.metacarpal.swing.y - finger_last.metacarpal.swing.y) * kStabilityFingerMCPSwing * c);



	helper.AddValue((finger.metacarpal.twist - finger_last.metacarpal.twist) * kStabilityFingerMCPTwist * c);



	helper.AddValue((finger.proximal_swing.x - finger_last.proximal_swing.x) * kStabilityFingerPXMSwingX * c);
	helper.AddValue((finger.proximal_swing.y - finger_last.proximal_swing.y) * kStabilityFingerPXMSwingY * c);

	helper.AddValue((finger.rots[0] - finger_last.rots[0]) * kStabilityCurlRoot * c);
	helper.AddValue((finger.rots[1] - finger_last.rots[1]) * kStabilityCurlRoot * c);
...
{
	return hand.finger[idx].metacarpal.swing.x + hand.finger[idx].proximal_swing.x + hand.finger[idx].rots[0] +
	       hand.finger[idx].rots[1];
}
                         ...
                         ResidualHelper<T> &helper)
{
	...
	helper.AddValue((hand.thumb.metacarpal.swing.y - last_hand.thumb.metacarpal.swing.y) * kStabilityThumbMCPSwing *
	                c);
	helper.AddValue((hand.thumb.metacarpal.twist - last_hand.thumb.metacarpal.twist) * kStabilityThumbMCPTwist * c);

	helper.AddValue((hand.thumb.rots[0] - last_hand.thumb.rots[0]) * kStabilityCurlRoot * c);
	helper.AddValue((hand.thumb.rots[1] - last_hand.thumb.rots[1]) * kStabilityCurlRoot * c);
	...
	for (int finger_idx = 0; finger_idx < 4; finger_idx++) {
		...
		computeResidualStability_Finger(finger, finger_last, state, helper);
	}
}

template <typename T>
void
computeResidualPlausibility(const OptimizerHand<T> &hand, KinematicHandLM &state, ResidualHelper<T> &helper)
{
#ifdef USE_HAND_PLAUSIBILITY
	T index_middle = (getFingerOverallCurl(hand, 0) - getFingerOverallCurl(hand, 1)) *
	                 kPlausibilityCurlSimilarity_IndexMiddle * T(state.input->plausibility_factor);
	T middle_ring = (getFingerOverallCurl(hand, 1) - getFingerOverallCurl(hand, 2)) *
	                kPlausibilityCurlSimilarity_MiddleRing * T(state.input->plausibility_factor);
	T ring_little = (getFingerOverallCurl(hand, 2) - getFingerOverallCurl(hand, 3)) *
	                kPlausibilityCurlSimilarity_RingLittle * T(state.input->plausibility_factor);
	helper.AddValue(index_middle);
	helper.AddValue(middle_ring);
	helper.AddValue(ring_little);
#endif

	for (int i = 0; i < 4; i++) {
		const OptimizerMetacarpalBone<T> &mcp = hand.finger[i].metacarpal;

		T cfac = T(kPlausibilityNeutralFingerMetacarpals * state.input->neutral_finger_metacarpals_factor);

		T x = mcp.swing.x * T(1.0) * cfac;
		T y = mcp.swing.y * T(0.5) * cfac;
		T z = mcp.twist * T(0.3) * cfac;
		helper.AddValue(x);
		helper.AddValue(y);
		helper.AddValue(z);
	}
		...
		for (int outer_joint = 0; outer_joint < 5; outer_joint++) {
			...
			for (int inner_finger = 0; inner_finger < 5; inner_finger++) {
				for (int inner_joint = 0; inner_joint < 4; inner_joint++) {
					...
					helper.AddValue(err * T(50) * T(state.input->no_collide_factor));
				}
			}
		}
	}
...
// But then later this just becomes a waste.
	...
	computeResidualPlausibility(hand, state, helper);
