...
palm_facing_head(handed_ handed)
{
	...
	math_quat_rotate_vec3((xrt_quat *)&hand->palm.orientation, (xrt_vec3 *)&vec3_forward,
	                      (xrt_vec3 *)&palm_direction);
...
static void
text_from_vec3(sk::vec3 at, float size, const char *hi)
{
	vec3 left_position = hierarchy_to_world_point(at);
	...
	pose_t pose;
	pose.position = (left_position * .9) + (head_position * .1);
	pose.orientation = input_head()->orientation; // quat_lookat(left_position, head_position);
...
ruler_window()
{
	...
	ui_handle_begin("Ruler", window_pose, bounds_t{vec3_zero, vec3{30 * cm2m, 4 * cm2m, 1 * cm2m}}, true,
	                ui_move_exact);
	color32 color = color_to_32(color_hsv(0.6f, 0.5f, 1, 1));
	text_add_at("Centimeters",
	            matrix_trs(vec3{14.5f * cm2m, -1.5f * cm2m, -0.6f * cm2m}, quat_identity, vec3{0.3f, 0.3f, 0.3f}),
	            0, text_align_bottom_left);
	for (int d = 0; d <= 60; d++) {
		float x = d / 2.0f;
		float size = (d % 2 == 0) ? 1.0f : 0.15f;
		line_add(vec3{(15 - x) * cm2m, 2 * cm2m, -0.6f * cm2m},
		         vec3{(15 - x) * cm2m, (2 - size) * cm2m, -0.6f * cm2m}, color, color, 0.5f * mm2m);

		if (d % 2 == 0 && d / 2 != 30) {
			text_add_at(std::to_string(d / 2).c_str(),
			            matrix_trs(vec3{(15 - x - 0.1f) * cm2m, (2 - size) * cm2m, -0.6f * cm2m},
...
static void
draw_camera_axis_at_pose(const sk::vec3 &vec, sk::quat ori, const char *name)
{
...
static void
draw_hand_axis_at_pose(const sk::pose_t &pose, const char *name)
{
	// We could take the matrix and plop it into a sk::matrix,
	// But that's a pretty annoying idea.
	// Basically I REALLY don't trust SK's matrix memory layout, and I kinda don't
...
static sk::pose_t
sk_from_xrt(xrt_pose pose)
{
	sk::pose_t pose_sk;
	pose_sk.position = *(sk::vec3 *)&pose.position;
	pose_sk.orientation = *(sk::quat *)&pose.orientation;
	return pose_sk;
}

static void
disp_xrt_hand(xrt_hand_joint_set &hand)
{
	// Draw axes
	for (int finger_idx = 0; finger_idx < 5; finger_idx++) {
		...
		for (int bone_idx = 0; bone_idx < 5; bone_idx++) {
			int idx = joints_5x5_to_26[finger_idx][bone_idx];

			draw_hand_axis_at_pose(sk_from_xrt(hand.values.hand_joint_set_default[idx].relation.pose),
		...
		for (int bone_idx = 0; bone_idx < 4; bone_idx++) {
			int idx = joints_5x5_to_26[finger_idx][bone_idx];
			int idx1 = joints_5x5_to_26[finger_idx][bone_idx + 1];

			sk::vec3 joint0 = sk_from_xrt(hand.values.hand_joint_set_default[idx].relation.pose).position;
			sk::vec3 joint1 = sk_from_xrt(hand.values.hand_joint_set_default[idx1].relation.pose).position;
