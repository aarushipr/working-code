...
template <typename T>
void
OptimizerHandUnpackFromVector(const T *in, T hand_size, HandLimit the_limit, OptimizerHand<T> &out)
{
...
#else
	out.hand_size = T(0.095);
#endif
}

template <typename T>
void
OptimizerHandPackIntoVector(OptimizerHand<T> &in, HandLimit the_limit, T *out)
{
	size_t acc_idx = 0;
...
OptimizerHandInit(OptimizerHand<T> &opt)
{
	...
	for (int i = 0; i < 4; i++) {
		...
		opt.finger[i].metacarpal.swing.y = T(0);
