...
def remove_orphans_of_datatype(dt):
    for obj in dt:
        if obj.users == 0:
            print(f"Found orphan object in {dt} with name {obj.name}! Purging!")
            dt.remove(obj)
...
def make_render_output(st: header.State, make_alpha_output):
    ...
    file_output_rgb.base_path = st.pb_response.output_color_images_folder
    ...
    file_output_alpha.base_path = st.pb_response.output_alpha_images_folder
