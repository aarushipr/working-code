...
private:
	...
	HandScalar store_wrist_location[3];
	HandScalar store_wrist_orientation_aax[3];
...
public:
	...
	wrist_location()
	{
		return store_wrist_location;
	}

	inline HandScalar *
	wrist_orientation_aax()
	{
		return store_wrist_orientation_aax;
	}

	inline OptimizerThumb<HandScalar> &
	thumb()
	{
		return store_thumb;
	}
...
public:
	...
	HandLimit()
	{
		...
		thumb_mcp_swing_x = {rad<double>(-60), rad<double>(60)};
		thumb_mcp_swing_y = {rad<double>(-60), rad<double>(60)};
		thumb_mcp_twist = {rad<double>(-90), rad<double>(90)};

		for (int i = 0; i < 2; i++) {
			thumb_curls[i] = {rad<double>(-90), rad<double>(40)};
		}
...
#undef margin
		...
		for (int finger_idx = 0; finger_idx < 4; finger_idx++) {
			...
			finger.mcp_swing_x = {rad<double>(-10), rad<double>(10)};
			finger.mcp_twist = {rad<double>(-4), rad<double>(4)};

			finger.pxm_swing_x = {rad<double>(-100), rad<double>(20)}; // ??? why is it reversed
			finger.pxm_swing_y = {rad<double>(-20), rad<double>(20)};

			for (int i = 0; i < 2; i++) {
				finger.curls[i] = {rad<double>(-90), rad<double>(10)};
			}
		}
	}
};
...
template <typename HandScalar>
HandScalar
LMToModel(HandScalar lm, minmax mm)
{
	return mm.min + ((sin(lm) + HandScalar(1)) * ((mm.max - mm.min) * HandScalar(.5)));
}

template <typename HandScalar>
HandScalar
ModelToLM(HandScalar model, minmax mm)
{
...
template <typename HandScalar>
void
OptimizerHandUnpackFromVector(const HandScalar *in, OptimizerHand<HandScalar> &out)
{
...
#endif
#ifdef USE_HAND_ORIENTATION
	out.wrist_orientation_aax()[0] = in[acc_idx++];
	out.wrist_orientation_aax()[1] = in[acc_idx++];
	out.wrist_orientation_aax()[2] = in[acc_idx++];
#endif
...
template <typename HandScalar>
void
OptimizerHandPackIntoVector(OptimizerHand<HandScalar> &in, HandScalar *out)
{
	size_t acc_idx = 0;
#ifdef USE_HAND_SIZE
...
#endif
#ifdef USE_HAND_ORIENTATION
	out[acc_idx++] = in.wrist_orientation_aax()[0];
	out[acc_idx++] = in.wrist_orientation_aax()[1];
	out[acc_idx++] = in.wrist_orientation_aax()[2];
#endif
#ifdef USE_EVERYTHING_ELSE
...
template <typename T>
void
OptimizerHandInit(OptimizerHand<T> &opt)
{
	opt.was_initialized = true;
	opt.hand_size() = (T)(0.095);

	// opt.wrist_orientation_aax()[0] = rad<double>(3.8);
	// opt.wrist_orientation_aax()[1] = rad<double>(2);
	// opt.wrist_orientation_aax()[2] = rad<double>(40);

	opt.wrist_location()[0] = (T)(0);
	opt.wrist_location()[1] = (T)(0);
	opt.wrist_location()[2] = (T)(0);
	...
	opt.template finger<0>().metacarpal().swing()[1] = (T)(-0.19);
	opt.template finger<1>().metacarpal().swing()[1] = (T)(0);
	opt.template finger<2>().metacarpal().swing()[1] = (T)(0.19);
	opt.template finger<3>().metacarpal().swing()[1] = (T)(0.38);

	opt.template finger<0>().proximal_swing()[1] = (T)(-0.01);
	opt.template finger<1>().proximal_swing()[1] = (T)(0);
	opt.template finger<2>().proximal_swing()[1] = (T)(0.01);
	opt.template finger<3>().proximal_swing()[1] = (T)(0.02);
}
